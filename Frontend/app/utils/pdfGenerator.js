/**
 * @description PDF generation utilities for application documents
 * Provides helper functions for creating professional PDFs like invoices and reports
...
function generateBookingInvoice(booking, options = {}) {
  ...
  const formatCurrency = (amount) => {
    // Format the number with commas according to Indian numbering system
    const formattedNumber = parseFloat(amount).toLocaleString('en-IN', {
      minimumFractionDigits: 2, 
      maximumFractionDigits: 2
    });
    // Use "Rs." instead of the Unicode symbol as a fallback
  ...
  // Invoice details box - wider box to accommodate full IDs
  ...
  doc.text(formatDate(new Date()), 185, y + 11, { align: "right" });
  doc.text(booking._id, 185, y + 17, { align: "right" });
  
  // Customer details - adjusted to fit beside invoice details
  ...
  // Add a second row for additional car pricing details
  y += 32;
  doc.setFillColor(245, 245, 245);
  doc.rect(20, y, 170, 18, 'F');
  
  doc.setFont("helvetica", "bold");
  doc.text("Distance:", 22, y + 5);
  doc.text("Price per km:", 22, y + 11);
  doc.text("Late fee:", 100, y + 5);
  
  doc.setFont("helvetica", "normal");
  doc.text(`${booking.distanceTravelled || 0} km`, 70, y + 5);
  doc.text(formatCurrency(booking.car.pricePerKm), 70, y + 11);
  doc.text(`${booking.car.finePercentage || 50}% of daily rate`, 140, y + 5);
  
  // Show outstation charges if applicable
  if (booking.tripType === 'outStation') {
    doc.setFont("helvetica", "bold");
    doc.text("Outstation:", 100, y + 11);
    doc.setFont("helvetica", "normal");
    doc.text(`${formatCurrency(booking.car.outStationCharges)}/day`, 150, y + 11);
  }
  
  y += 30; // Adjusted to account for the new row
  ...
  // Base rental (shown for reference but not included in calculation)
