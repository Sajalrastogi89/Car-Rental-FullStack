myApp.controller("userChatController", ["$scope", "chatService", "$timeout", 
  function ($scope, chatService, $timeout) {
    ...
    $scope.userId = userData.id || userData.email;
    ...
    $scope.getAllChats = function() {
      return chatService.getChats("user_id")
        .then((allChats) => {
    ...
    function sendTextMessage(messageText) {
      // Get receiver ID from selected chat
      const receiverId = $scope.selectedChat.user?.id || 
